...
const sectionObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if(!hasScrolled) return; // ما نعملش شيء قبل أول scroll
...
// ======== Intersection Observer للـ Tab Items (للانيميشن فقط) ========
const tabObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if(!hasScrolled) return;
    if(entry.isIntersecting && entry.intersectionRatio > 0.5) {
      entry.target.classList.add("show"); // للانيميشن فقط
    }
  });
}, { threshold: 0.5 });
...
// ======== تفعيل الزرار عند الضغط (Active يتحكم فيه الزرار فقط) ========
